# OpenWhisper — User Guide

OpenWhisper types what you say. You put the cursor in a text box — a chat,
an email, a document — and speak. Your words appear where the cursor is,
and in chat apps OpenWhisper can even press Return for you. There are no
buttons to hold and no keyboard shortcuts to remember: you control
everything with your voice.

Everything happens on your Mac. Your voice is never sent anywhere.

This guide assumes no technical background. When it says *menu bar*, it
means the strip along the top of your screen with the clock; when it says
*the OpenWhisper menu*, it means what opens when you click the small
**waveform icon** (a few vertical sound bars) in that strip.
...
2. **Accessibility** — this is what lets it *type* into other apps. macOS
...
The waveform icon in the menu bar tells you at a glance what's happening:

| Icon | Meaning |
|------|---------|
| 🟢 Green waveform | Listening. Speak and it types. |
| 🔴 Red filled waveform | Hearing your voice right now. |
| 🟠 Orange waveform with a slash | **Muted.** Hears only the commands "start listening" and "Hey Claude". |
| Grey waveform with a slash | Microphone fully off. |
| Hourglass / arrow | Starting up or downloading the speech model. |

You may also see a **separate orange microphone icon** appear in the menu
bar. That one belongs to **macOS**, not OpenWhisper — the system shows it
whenever *any* app is using the microphone, and clicking it reveals
system audio options. It appears for Zoom and FaceTime too.
...
- **In Claude and Perplexity**, OpenWhisper presses Return for you about a
  second after you stop talking — a hands-free conversation. In every
  other app it only types; say **"send it"** when you want Return pressed.
...
| Say | What happens |
|-----|--------------|
| "read it to me" / "read this" | Reads the front window's text aloud — any app. |
...
| Say | What happens |
|-----|--------------|
| "replace ⟨wrong⟩ with ⟨right⟩" | From now on, whenever it hears ⟨wrong⟩ it types ⟨right⟩. Example: "replace kubernetes with Kubernetes". |
...
| "mac mic" / "apple mic" | Use the Mac's built-in microphone (recommended — headphone mics reduce audio quality). |
...
- **Turn Mic Off Completely** — releases the microphone entirely (the
  system's orange mic indicator goes out). Use this for total certainty.
- **Auto-send** — whether Return is pressed after you pause.
- **Type into any app** — on: dictate anywhere; off: only into your
  allowed apps (Claude, Perplexity by default).
...
- **Languages** — your main dictation language, and optionally a second
  one. With a second language set, you can switch languages just by
  speaking: each sentence goes to the language it was spoken in, no
  command needed. (Off by default — it uses more battery.)
...
Check the icon: orange = muted (say "start listening"); grey = mic off
(menu → Turn Mic Back On). Green but nothing appears? Make sure a text
box is focused and the cursor is blinking. Still nothing: the menu will
show **⚠️ Grant Accessibility Access** if macOS revoked the typing
...
Say "replace ⟨what it typed⟩ with ⟨what you meant⟩" — permanent, and
editable later in Settings.

**I can't find the icon.**
A crowded menu bar (especially on Macs with a notch) hides icons. Hold
⌘ and drag other menu bar icons off (or remove some in System Settings →
Control Center) to make room.
...
[README.md](../README.md) for building, tests, and design notes. If it
